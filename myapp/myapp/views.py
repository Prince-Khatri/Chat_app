...
def home(request):
    ...
    return render(request,'ui_v1/home.html',context)
...
def login(request):
    ...
    return render(request,'ui_v1/login.html', context)
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.close()
            conn.close()
    return render(request,'ui_v1/signup.html',context)
...
def chat_home(request):
    ...
    return render(request,'ui_v1/chat_home.html', context)
...
def get_contacts(request):
    """Get all contacts (users and groups) for the current user"""
    user = get_current_user(request)
    if not user:
        return JsonResponse({'status': 'error', 'message': 'Unauthorized'})
    ...
    try:
        ...
        cursor.execute('''
            SELECT User_Id as id, Name as name, 'user' as type
            FROM User 
            WHERE User_Id != %s
            ORDER BY Name
        ''', (user['User_Id'],))
        ...
        cursor.execute('''
            SELECT g.group_id as id, g.group_name as name, 'group' as type
            FROM group_table g
            INNER JOIN user_group ug ON g.group_id = ug.group_id
            WHERE ug.user_id = %s
            ORDER BY g.group_name
